...
These files contain translation strings used in SleepPro.

## Contributing translations (non-English)
To contribute translations, please use the [Crowdin Translation Page](https://vk.com/sleepwpro), select the language you want to translate and go to SleepPro and select "PocketMine core strings". There may be multiple branches available - if in doubt, stick with `stable`.
...
- Vanilla strings should use the same keys as used by [Mojang](https://raw.githubusercontent.com/Mojang/bedrock-samples/refs/heads/main/resource_pack/texts/en_US.lang). However, SleepPro currently uses `{%paramName}` for parameters instead of `%1$s` `%2$s` etc, so you can't copy-paste them directly. Make sure to adapt these.
- Strings specifically for SleepPro can have any keys you like, but they must start with `pocketmine.`
